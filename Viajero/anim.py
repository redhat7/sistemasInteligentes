...
def framec( solution, nc ):
    ...
    plt.title('{} Puntos de reparto, 5 Algoritmos de busqueda'.format(nc))
...
def frame0(solution, nodes, l, title):
    ...
    plt.title('{}  Tamanio de camino: {:.1f}'.format(title,l))
    plt.savefig( ("%05d" % pic)+'.png')
    plt.clf()
    print pic
    pic += 1
...
def frame(nodes, solution, sn, t, c, y, x, z, gain):
    ...
    plt.title('(4)  SA Temp {:4.1f} Mejor camino {:6.1f}\nSwaps {}  Gain {:12.2f} '.format(t,l_min,nn,gain))
    plt.savefig( ("%05d" % pic)+'.png')
    plt.clf()
    pic += 1
    print pic
...
def frame4(nodes, solution, sn, c, y, x, z, gain):
    ...
    plt.title('(3)  Viaje 2-Opt  {:6.1f}'.format(l_min))
    plt.savefig( ("%05d" % pic)+'.png')
    plt.clf()
    pic += 1
    print pic
...
def greedy_algorithm(nodes):
    # Greedy Algorithm
    print 'Analizando el camino greedy'
...
def create_animation(nodes):
    ...
    if do_intro:
        ...
        for i in range(2, number_of_nodes):
            s = solution0[0:i]
            frame0(s, nodes, total_length(nodes, s), "(1)  Ruta random")
        s = solution0
        for i in range(60):
            frame0(s, nodes, total_length(nodes, s), "(1)  Ruta random")
        ...
        for i in range(2, number_of_nodes):
            s = solution[0:i]
            frame0(s, nodes, total_length(nodes, s), "(2)  Busqueda Greedy")
        s = solution
        for i in range(60):
            frame0(s, nodes, total_length(nodes, s), "(2)  Busqueda Greedy")
